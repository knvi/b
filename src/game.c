...
void game_tick(game *g)
{
    ...
    if(g->online) {
                    ...
                    while(data_position < data_size) {
                        switch(g->buffer[data_position]) {
                            ...
                            case SPAWN_PLAYER_ID:
                            {
                                ...
                                g->w.num_players++;
                                printf("Player %d joined the game", packet->player_id);
                            ...
                            case POSITION_UPDATE_ID:
                            {
                                ...
                                for(int i = 0; i < g->w.num_players; i++) {
                                    if(g->w.players[i].id == packet->player_id) {
                                        g->w.players[i].x = htons(packet->x);
                                        g->w.players[i].y = htons(packet->y);
                                        g->w.players[i].z = htons(packet->z);
                                        break;
                                    }
                                }
        ...
        if(g->w.block_changed) {
            ...
            send(g->server_socket, (char*) &packet, sizeof(packet), 0);
        }
